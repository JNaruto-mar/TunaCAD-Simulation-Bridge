...
assert.deepEqual(pipeline.providerV2?.capabilities.study.interactionTypes, ['bonded_tie', 'shared_topology', 'rigid_connector', 'frictionless_contact']);
assert.equal(pipeline.providerV2?.capabilities.fieldResults.paginated, true);
assert.deepEqual(pipeline.providerV2?.capabilities.analysisTypes, ['linear_static', 'modal', 'linear_buckling', 'static_contact']);
