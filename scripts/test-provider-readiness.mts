...
assert.deepEqual(pipeline.providerV2?.capabilities.study.interactionTypes, ['bonded_tie', 'shared_topology', 'rigid_connector']);
assert.equal(pipeline.providerV2?.capabilities.fieldResults.paginated, true);
assert.deepEqual(pipeline.providerV2?.capabilities.analysisTypes, ['linear_static', 'modal']);
assert.deepEqual(pipeline.providerV2?.capabilities.study.modal, { maximumModes: 24, frequencyBounds: true, massFormulations: ['consistent'], constrainedOnly: true });
