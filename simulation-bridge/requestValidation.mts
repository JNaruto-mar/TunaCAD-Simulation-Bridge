...
const references = z.array(text).length(1);
const face = z.object({
...
const schema = z.object({
  ...
  geometry: z.object({
    ...
    references: z.array(z.object({ semanticReferenceId: text, ownerPartId: text, geometryKind: z.literal('FACE'), role: z.enum(['load', 'constraint']), sourceFeatureId: text.nullable(), resolutionState: z.literal('valid'), resolvedAtProjectRevision: text, face }).strict()).length(2),
  }).strict(),
  ...
  loads: z.array(z.object({ id: text, name: text, type: z.literal('surface_force'), semanticReferenceIds: references, forceN: vector }).strict()).length(1),
  constraints: z.array(z.object({ id: text, name: text, type: z.literal('fixed'), semanticReferenceIds: references }).strict()).length(1),
...
export function validateRequest(value: unknown, now = Date.now()): NeutralSimulationRequest {
  ...
  const fixed = request.constraints[0].semanticReferenceIds[0];
  const loaded = request.loads[0].semanticReferenceIds[0];
  if (fixed === loaded) throw new Error('BRIDGE_REFERENCE_INVALID');
  for (const [id, role] of [[fixed, 'constraint'], [loaded, 'load']]) {
